...
<section class="sheet">
        <?php
            ...
            // $transaksi = mysqli_query($db_conn, "SELECT payment.*, sales.code FROM `payment` JOIN sales ON sales.id=payment.sales_id WHERE payment.id=52 AND payment.deleted=0");
            // $code = "";
            // $date = "";
            // $sname = "";
    ...
    <div   style="display: flex;padding: 5px 10px 0 10px;">
        <div style="width: 100%;padding-right: 10px;" class="col-md-16">
            ...
            <table width="100%" >
                <tbody>
                <?php
                    $transaksi = mysqli_query($db_conn, "SELECT payment.*, sales.code FROM `payment` JOIN sales ON sales.id=payment.sales_id WHERE payment.id='$id' AND payment.deleted=0");
                    $tot = 0;
                    $i = 1;
                    while($row=mysqli_fetch_assoc($transaksi)){
                        echo '<tr><td style="border: 1px solid black;" class="text-center"><b>Pembayaran Sejumlah</b></td><td style="border: 1px solid black;" class="text-left"><b>'.rupiah($row['amount']).'</b></td></tr>';
                        echo '<tr><td style="border: 1px solid black;" class="text-center"><b>Untuk No. Nota</b></td><td style="border: 1px solid black;" class="text-left"><b>'.$row['code'].'</b></td></tr>';
                        echo '<tr><td style="border: 1px solid black;" colspan="2" class="text-center"><center><b>TELAH DI TERIMA</b></center></td></tr>';
