...
$menu = mysqli_query($db_conn, "SELECT sa.id, sa.code, sa.notes, sa.created_at, sa.paid_off, s.name AS sName, e.name as eName FROM sales sa JOIN shops s ON sa.shop_id = s.id JOIN employees e ON sa.created_by = e.id WHERE sa.deleted = 0 AND sa.paid_off = 1 AND DATE(sa.created_at) BETWEEN '$dateFrom' AND '$dateTo' ORDER BY sa.id DESC");
...
if (mysqli_num_rows($menu) > 0) {
    ...
    array_push($data,$all);
    echo json_encode(["success" => 1, "grossprofit" => $res]);
} else {
