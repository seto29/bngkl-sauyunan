...
<body>
    ...
    <?php
      ...
      header("Content-Disposition: attachment; filename=Daftar_Pembayaran_Pembelian\"$dateFormat\".xls");
      ...
      $str = date('d-m-Y', strtotime($dateFrom))." Sampai Dengan ". date('d-m-Y', strtotime($dateUntil));
        ...
        while($row = mysqli_fetch_array($list)){
            ...
            if($i==0){
                ...
                if($temp!=$row['kode_pembelian']){
                    array_push($array, $row);
                }
            }
            $temp= $row['kode_pembelian'];
            $i+=1;
        }
	?>
 	<center>
  ...
  <br/>
        ...
        foreach ($array as $value) {
            if($first==false && $temp!=$value['tanggal_beli'] ){
                ?>
